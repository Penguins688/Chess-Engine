...
void printChessboard(const std::vector<std::vector<std::shared_ptr<piece>>>& chessboard) {
    ...
    std::cout << "  ";
    for (int col = 0; col < numCols; ++col) std::cout << col << " ";
    std::cout << std::endl;

    for (int row = 0; row < numRows; ++row) {
        std::cout << row << " "; 
        for (const auto& cell : chessboard[row]) {
            if (cell) std::cout << cell->name << " ";
            else std::cout << ". ";
        }
        std::cout << std::endl;
    }
...
int main() {
    ...
    while (true) {
        ...
        auto [whiteEval, blackEval] = evaluate(rawChessboard);
        std::cout << "White evaluation:" << whiteEval << "\n" << "Black evaluation: " << blackEval << std::endl;

        if (player == "b") {
            std::cout << "Bot is moving..." << std::endl;
            std::pair<std::pair<int, int>, std::pair<int, int>> values = move(rawChessboard, "b");
            movePiece(values.first.second, values.first.first, values.second.second, values.second.first);
